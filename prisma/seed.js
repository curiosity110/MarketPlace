...
const templatesByCategorySlug = {
  cars: [
    ...
    { key: "fuel", label: "Fuel", type: CategoryFieldType.SELECT, optionsJson: JSON.stringify(["Petrol", "Diesel", "Hybrid", "Electric"]), required: true, order: 5 },
    { key: "transmission", label: "Transmission", type: CategoryFieldType.SELECT, optionsJson: JSON.stringify(["Manual", "Automatic"]), required: true, order: 6 },
  ],
  "real-estate": [
    ...
    { key: "heating", label: "Heating", type: CategoryFieldType.SELECT, optionsJson: JSON.stringify(["Central", "Electric", "Wood", "Gas"]), required: false, order: 5 },
  ],
  jobs: [
    ...
    { key: "contract", label: "Contract", type: CategoryFieldType.SELECT, optionsJson: JSON.stringify(["Full-time", "Part-time", "Contract", "Internship"]), required: true, order: 5 },
  ],
  phones: [
    { key: "brand", label: "Brand", type: CategoryFieldType.TEXT, required: true, order: 1 },
    { key: "model", label: "Model", type: CategoryFieldType.TEXT, required: true, order: 2 },
    { key: "storage", label: "Storage", type: CategoryFieldType.SELECT, optionsJson: JSON.stringify(["64GB", "128GB", "256GB", "512GB"]), required: true, order: 3 },
    { key: "condition", label: "Condition", type: CategoryFieldType.SELECT, optionsJson: JSON.stringify(["New", "Used", "Refurbished"]), required: true, order: 4 },
  ...
  services: [
    { key: "serviceType", label: "Service type", type: CategoryFieldType.SELECT, optionsJson: JSON.stringify(["Repair", "Cleaning", "Consulting", "Transport"]), required: true, order: 1 },
    { key: "availability", label: "Availability", type: CategoryFieldType.TEXT, required: false, order: 2 },
  ],
  furniture: [
    { key: "material", label: "Material", type: CategoryFieldType.SELECT, optionsJson: JSON.stringify(["Wood", "Metal", "Plastic", "Glass"]), required: true, order: 1 },
    { key: "dimensions", label: "Dimensions", type: CategoryFieldType.TEXT, required: false, order: 2 },
    { key: "color", label: "Color", type: CategoryFieldType.SELECT, optionsJson: JSON.stringify(["White", "Black", "Brown", "Gray"]), required: false, order: 3 },
  ],
  fashion: [
    { key: "size", label: "Size", type: CategoryFieldType.SELECT, optionsJson: JSON.stringify(["XS", "S", "M", "L", "XL"]), required: true, order: 1 },
    { key: "brand", label: "Brand", type: CategoryFieldType.TEXT, required: false, order: 2 },
    { key: "color", label: "Color", type: CategoryFieldType.SELECT, optionsJson: JSON.stringify(["Black", "White", "Blue", "Red", "Green"]), required: false, order: 3 },
  ],
};
...
async function main() {
  const upsertedCategories = await Promise.all(
    categories.map(([name, slug]) => prisma.category.upsert({ where: { slug }, update: { name, isActive: true }, create: { name, slug } })),
  );
